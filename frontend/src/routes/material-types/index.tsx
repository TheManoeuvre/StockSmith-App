...
function MaterialTypesList() {
  ...
  const { data, isLoading, error } = useQuery({ queryKey: ["material-types"], queryFn: materialTypesApi.list });
  const [name, setName] = useState("");

  const createMutation = useMutation({
    mutationFn: () => materialTypesApi.findOrCreate(name.trim()),
    onSuccess: () => {
  ...
  if (isLoading) return <p>Loading material types…</p>;
  if (error) return <p className="text-red-600">{(error as Error).message}</p>;

  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-xl font-semibold">Material Types</h1>
      ...
      <table className="w-full border-collapse bg-white text-left text-sm shadow-sm">
        ...
        <tbody>
          {data?.map((mt) => (
            <tr key={mt.id} className="border-b border-slate-100">
              <td className="p-2">{mt.name}</td>
            </tr>
          ))}
        </tbody>
      </table>
