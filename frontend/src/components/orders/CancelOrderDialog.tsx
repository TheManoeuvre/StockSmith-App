...
export function CancelOrderDialog({
  ...
  const [decisions, setDecisions] = useState<
    Record<number, LineCancellationDecision>
  >({});
  ...
  useEffect(() => {
    ...
    for (const line of preview.lines) {
      initial[line.order_line_id] = {
        ...
        kitting_disposition:
          line.shipped_qty > 0 ? line.default_kitting_disposition : undefined,
      };
    }
  ...
  const cancelMutation = useMutation({
    mutationFn: () =>
      ordersApi.cancel(orderId, {
        line_decisions: Object.values(decisions),
        reason: reason || null,
      }),
    onSuccess: () => {
  ...
  return (
    <Modal
      title="Cancel order"
      maxWidth="max-w-2xl"
      // Escape/backdrop must not dismiss mid-cancel — the mutation is already committing.
      onClose={cancelMutation.isPending ? () => {} : onClose}
      footer={
        <>
          <button
            onClick={onClose}
    ...
    >
      ...
      {preview && preview.lines.length === 0 && (
        <p className="text-sm text-slate-600">
          Nothing allocated or shipped on this order — it'll just be marked
          cancelled.
        </p>
      )}
        ...
        {preview?.lines.map((line) => (
          ...
          >
            ...
            {line.pending_qty > 0 && (
              <div className="mt-2 text-sm">
                <p className="text-slate-500">
                  {line.pending_qty} unit(s) reserved, not yet shipped
                </p>
                <DispositionRadio
            ...
            {line.shipped_qty > 0 && (
              <div className="mt-2 text-sm">
                <p className="text-slate-500">
                  {line.shipped_qty} unit(s) already shipped
                </p>
                <DispositionRadio
                ...
                {line.kitting_materials.length > 0 && (
                  <div className="mt-2 rounded bg-slate-50 p-2">
                    <p className="text-xs text-slate-500">
                      Kitting:{" "}
                      {line.kitting_materials
                        .map((m) => `${m.material_name} ×${m.qty}`)
                        .join(", ")}
                    </p>
                    <DispositionRadio
                      ...
                      onChange={(v) =>
                        setKittingDisposition(line.order_line_id, v)
                      }
