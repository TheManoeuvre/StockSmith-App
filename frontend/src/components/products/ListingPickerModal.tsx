...
export function ListingPickerModal({
  ...
  const [selected, setSelected] = useState<EligibilityAnnotatedCandidate | null>(null);
  ...
  const { data: proposal, isLoading: proposalLoading } = useQuery({
    queryKey: ["platforms", "ebay", "variation-mapping", chosenProductId, selected?.external_listing_id],
    queryFn: () => platformsApi.fetchVariationMapping(chosenProductId!, selected!.external_listing_id),
  ...
  const effectiveMapping = useMemo(() => {
    ...
    for (const entry of proposalEntries) {
      const key = entry.variant_id === null ? "product" : String(entry.variant_id);
  ...
  const selectableSkus = useMemo(() => {
    const fromProposal = proposalEntries.map((e) => e.matched_sku).filter((s): s is string => !!s);
    return fromProposal.length > 0 ? Array.from(new Set(fromProposal)) : selected?.skus ?? [];
  }, [proposalEntries, selected]);

  const mappingComplete =
    proposalEntries.length > 0 &&
    proposalEntries.every((e) => effectiveMapping[e.variant_id === null ? "product" : String(e.variant_id)]);

  const adoptMutation = useMutation({
    mutationFn: () => {
      if (!selected || chosenProductId === null) throw new Error("No listing or product selected");
      const variation_mapping: VariationMappingChoice[] = proposalEntries.map((e) => ({
        variant_id: e.variant_id,
        sku: effectiveMapping[e.variant_id === null ? "product" : String(e.variant_id)],
      }));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded bg-white shadow-lg">
        <div className="border-b border-slate-200 p-4">
          <h2 className="text-lg font-semibold">Link an eBay listing</h2>
          {report && !selected && (
            <p className="text-sm text-slate-500">
              {report.eligible_count} of {report.total_count} unmigrated listing(s) look eligible to adopt
            </p>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
            ...
            <>
              ...
              <div className="flex flex-col gap-2">
                {report?.listings.length === 0 && (
                  <p className="text-sm text-slate-600">No unmigrated eBay listings found for this shop.</p>
                )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-500">
                        Item {candidate.external_listing_id} · qty {candidate.quantity} ·{" "}
          ...
          {selected && (
            <div className="flex flex-col gap-3">
              <p className="text-sm">
                Selected: <span className="font-medium">{selected.title}</span> (item {selected.external_listing_id})
              </p>

              {proposalLoading && <p className="text-sm text-slate-500">Loading listing detail from eBay…</p>}

              {proposalEntries.length > 0 && (
                <div className="flex flex-col gap-2 rounded border border-slate-200 p-3">
                  <p className="text-sm font-medium">Map StockSmith variants to eBay SKUs</p>
              ...
              {!done && (
                <>
                  <label className="flex items-start gap-2 rounded bg-slate-50 p-2 text-sm">
                    ...
                    <span>
                      ...
                      <span className="block text-xs text-slate-500">
                        Done before migrating, while the SKU is still editable. Leave this off and any mismatch is
                        only reported — you'd then have to fix it in Seller Hub yourself.
                      </span>
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-2 rounded border border-amber-300 bg-amber-50 p-2 text-sm text-amber-900">
                    <input
                      type="checkbox"
                      className="mt-0.5"
                      checked={confirmed}
                    ...
                    />
                    <span>
                      I understand this permanently converts the eBay listing to an Inventory API listing.
                      <span className="block text-xs">
                        This can't be undone from StockSmith or Seller Hub. The listing keeps its item number,
                        watchers and sales history{alignSkus ? ", and its SKUs will be rewritten first" : ""}.
                      </span>
                    </span>
                  </label>
                </>
              ...
              {adoptMutation.data && <AdoptionResultBanner result={adoptMutation.data} />}
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-slate-200 p-4">
          ...
          {selected && !done && (
            <button
              ...
              disabled={adoptMutation.isPending || !mappingComplete || !confirmed}
              className="rounded bg-slate-900 px-4 py-2 text-sm text-white disabled:opacity-50"
            >
              {adoptMutation.isPending ? "Migrating & linking…" : "Migrate & link"}
            </button>
          )}
        </div>
      </div>
  ...
  if (conflicts.length === 0) {
    return (
      <p className="rounded bg-green-50 p-2 text-sm text-green-800">
        Listing linked successfully{result.skus_aligned ? " — eBay's SKUs were rewritten to match StockSmith." : "."}
      </p>
    );
  }
  return (
    <div className="rounded bg-amber-50 p-2 text-sm text-amber-800">
      <p className="font-medium">Linked, but {conflicts.length} SKU(s) don't match StockSmith:</p>
      <ul className="mt-1 list-inside list-disc">
        {conflicts.map((u) => (
          <li key={u.variant_id ?? "product"}>
            expected <span className="font-mono">{u.expected_sku}</span>, eBay has{" "}
            <span className="font-mono">{u.actual_sku}</span>
          </li>
        ))}
      </ul>
      <p className="mt-1 text-xs">
        Quantity pushes for these units will fail until the SKUs match. Rename them in Seller Hub, then re-test sync.
      </p>
    </div>
  );
}
