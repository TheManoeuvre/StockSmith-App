"""Backfill cost_per_unit_snapshot/kitting_cost_per_unit_snapshot for OrderLines that were
already allocated (or shipped) before COGS snapshotting moved from line-creation time to
first-allocation time (see app/services/allocation.py::_allocate_line).
...
cost_per_unit_snapshot/kitting_cost_per_unit_snapshot stuck at 0/NULL forever, even after
the material's cost later became real — nothing in the codebase ever revisited it. This
script is the one-off remediation: it recomputes the snapshot, as if first-allocation
snapshotting had been in effect all along, for every already-allocated line still missing
one.

Only touches lines with allocated_qty > 0 (the analogue of "has been allocated at least
once") and both snapshot fields NULL — an unallocated line is correctly left alone, since
its real snapshot will be captured whenever it's actually allocated, and a line that
already has a snapshot (zero or otherwise) is left as-is rather than silently overwritten.
...
async def _find_candidates(session) -> list[OrderLine]:
    result = await session.execute(
        select(OrderLine).where(
            ...
            OrderLine.kitting_cost_per_unit_snapshot.is_(None),
...
async def run(apply: bool) -> int:
    async with async_session_factory() as session:
        ...
        for line in candidates:
            cost_per_unit, kitting_cost_per_unit = await compute_line_cost_snapshot(
                session, line.product_id, line.variant_id
            )
            still_unknown = cost_per_unit is None and kitting_cost_per_unit is None
            print(
                f"  order_line #{line.id} (order #{line.order_id}, product #{line.product_id}, "
                f"variant #{line.variant_id}): cost_per_unit={cost_per_unit} "
                f"kitting_cost_per_unit={kitting_cost_per_unit}"
                + ("  [still uncostable — no BOM at all]" if still_unknown else "")
            )
            if apply and not still_unknown:
                line.cost_per_unit_snapshot = cost_per_unit
                line.kitting_cost_per_unit_snapshot = kitting_cost_per_unit
                updated += 1
